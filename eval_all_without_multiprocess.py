...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def get_scene_models(self, scene_id, ann_id):
        ...
        scene_reader = xmlReader(os.path.join(self.root, 'scenes', get_scene_name(scene_id), self.camera, 'annotations',
                                              '%04d.xml' % (ann_id,)))
    ...
    def get_model_poses(self, scene_id, ann_id):
        ...
        **Output:**
        ...
        scene_reader = xmlReader(
            os.path.join(scene_dir, get_scene_name(scene_id), self.camera, 'annotations', '%04d.xml' % (ann_id,)))
        posevectors = scene_reader.getposevectorlist()
        obj_list = []
        pose_list = []
        for posevector in posevectors:
    ...
    def eval_scenes_without_multiprocess(self, scene_ids, dump_folder):
        '''
           with multiprocess, my code will be stuck, so this function will eval scenes without multiprocess,
        ans will save enery scene's acc in a .npy file
        :param scene_ids: list of int of scene index.
        :param dump_folder: string of the folder that saves the npy files.
        :return: scene_acc_list: list of the scene accuracy.
        '''
        acc_save_path = os.path.join(dump_folder, 'ap_scenes')
        if not os.path.exists(acc_save_path):
            os.makedirs(acc_save_path)

        scene_acc_list = []
        for scene_id in scene_ids:
            ans = self.eval_scene(scene_id, dump_folder)
            path = os.path.join(acc_save_path, str(scene_id).zfill(4) + '.npy')
            np.save(path, np.array(ans))
            print('\nsave {} successfully'.format(path))
            scene_acc_list.append(ans)
    ...
    def eval_all(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        res = np.array(self.eval_scenes_without_multiprocess(scene_ids=list(range(100, 190)), dump_folder=dump_folder))
        ap = [np.mean(res), np.mean(res[0:30]), np.mean(res[30:60]), np.mean(res[60:90])]
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Seen={}, AP Similar={}, AP Novel={}'.format
              (self.camera, ap[0], ap[1], ap[2], ap[3]))
        return res, ap
